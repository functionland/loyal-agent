...
if __name__ == "__main__":
                        ...
                        def generate():
                            ...
                            while True:
                                while len(global_text) > 0:
                                    ...
                                    # Construct a single response for this chunk
                                    response_chunk = {
                                        "id": "rkllm_chat",
                                        "object": "rkllm_chat",
                                        "created": None,
                                        "choices": [
                                            {
                                                ...
                                                "logprobs": None,
                                                "finish_reason": None if global_state == 0 else "stop",
                                            }
                                        ],
                                        "usage": {
                                            "prompt_tokens": None,
                                    ...
                                    # Yield the current chunk as a JSON string
                                    yield f"{json.dumps(response_chunk)}\n\n"

                                model_thread.join(timeout=0.005)
                                model_thread_finished = not model_thread.is_alive()
                                if model_thread_finished and len(global_text) == 0:
                                    break  # Exit only when thread is done AND no more chunks
    ...
    app.run(host='127.0.0.1', port=8083, threaded=True, debug=False)
